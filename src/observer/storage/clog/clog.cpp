...
RC CLogBuffer::append_log_record(CLogRecord *log_rec, int &start_off)
{
  ...
  if (log_block->log_block_hdr_.log_data_len_ == CLOG_BLOCK_DATA_SIZE) {  // 当前block已写满
    ...
    if (logrec_left_len <= (int)(CLOG_BLOCK_DATA_SIZE - log_block->log_block_hdr_.log_data_len_)) {  //不需要再跨block存放
...
RC CLogFile::recover(CLogMTRManager *mtr_mgr, CLogBuffer *log_buffer)
{
  ...
  while (read_size != 0) {
    ...
    while (buffer_offset < CLOG_REDO_BUFFER_SIZE) {
      ...
      while ((int)rec_offset < (int)CLOG_BLOCK_HDR_SIZE + log_block->log_block_hdr_.log_data_len_) {
      ...
      if ((int)log_block->log_block_hdr_.log_data_len_ < (int)CLOG_BLOCK_DATA_SIZE) {  //最后一个block
...
RC CLogFile::block_recover(CLogBlock *block, int16_t &offset, CLogRecordBuf *logrec_buf, CLogRecord *&log_rec)
{
  if (offset == CLOG_BLOCK_HDR_SIZE &&
    ...
    if (CLOG_BLOCK_SIZE - offset < (int)sizeof(CLogRecordHeader)) {  // 一定是跨block的第一部分
      // 此时无法确定log record的长度
      // 开始写入logrec_buf
