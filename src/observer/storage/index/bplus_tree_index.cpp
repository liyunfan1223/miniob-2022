...
//估计需要修改
  ...
  RC rc = index_handler_.create(file_name, field_meta[0]->type(), field_meta[0]->len()*field_meta.size());
  ...
  if (RC::SUCCESS != rc) {
    ...
    inited_ = true;
    LOG_INFO(
        "Successfully create index, file_name:%s, index:%s, field:%s", file_name, index_meta.name(), s.c_str());
...
RC BplusTreeIndex::open(const char *file_name, const IndexMeta &index_meta,  std::vector<FieldMeta *> &field_meta)
{
  ...
  LOG_INFO(
      "Successfully open index, file_name:%s, index:%s, field:%s", file_name, index_meta.name(), s.c_str());
  return RC::SUCCESS;
}
...
RC BplusTreeIndex::close()
{
  if (inited_) {
    std::string s="";
    for(auto f:index_meta_.field()){
      s+=f+" ";
    }
    LOG_INFO("Begin to close index, index:%s, field:%s",
        index_meta_.name(), s.c_str());
...
{
  return index_handler_.insert_entry(record + field_meta_[0].offset()*field_meta_.size(), rid);
}

RC BplusTreeIndex::delete_entry(const char *record, const RID *rid)
{
  return index_handler_.delete_entry(record +field_meta_[0].offset()*field_meta_.size(), rid);
}
